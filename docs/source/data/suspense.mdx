...
If your application renders multiple components that use the same `query` and `variables`, this may present a problem: the queries made by multiple hooks share the same identity causing them to **always suspend at the same time**, regardless of which component initiates or re-initiates a network request.
