...
# From here on it's AI, don't trust it blindly!

# Refined Apollo Client 4.0 Migration Guide Plan

This document outlines the key migration points that should be added or expanded in the migration guide, focusing on practical "before" and "after" examples that developers need when migrating from Apollo Client 3.x to 4.0.

## Priority 1: Critical Breaking Changes

### 1. Error Handling Overhaul

**What changed:** The `errors` array property has been removed from all results in favor of a single `error` property.

**Before:**

```ts
// Apollo Client 3.x
const { loading, errors, data } = useQuery(MY_QUERY);
...
**What changed:** The `ErrorLink` (previously created with `onError`) now uses a single `error` property instead of separate `graphQLErrors`, `networkError`, and `protocolErrors`.
...
### 3. Local State System Overhaul

**What changed:** Local state with `@client` fields now requires explicit opt-in with `LocalState` class.
...
resolvers: {
  Query: {
    myResolver: (parent, args, { cache, ...requestContext }) => {
      // cache was directly available
    };
  }
}
...
resolvers: {
  Query: {
    myResolver: (parent, args, { requestContext, client, phase }) => {
      const cache = client.cache;
      // phase is "exports" or "resolve"
    };
  }
}
```

## Priority 2: SetContextLink Argument Changes

### 4. SetContextLink Constructor Arguments

**What changed:** `SetContextLink` flips the callback arguments compared to `setContext` - `prevContext` comes first, `operation` comes second.
...
## Priority 3: Major React Hook Changes

### 5. useLazyQuery SSR and Options Changes

**What changed:** `useLazyQuery` no longer supports SSR and the `execute` function has limited options.

**Before:**

```ts
// Apollo Client 3.x
const [execute, { data, loading }] = useLazyQuery(MY_QUERY, {
  fetchPolicy: "network-only",
  context: { headers: { "X-Custom": "value" } },
});

// Could override options in execute
await execute({
  fetchPolicy: "cache-first",
  context: { headers: { "X-Other": "value" } },
  variables: { id: 1 },
});
```
...
const [execute, { data, loading }] = useLazyQuery(MY_QUERY, {
  fetchPolicy: "network-only",
  // context must be set here if static
});

// Only variables and context can be passed to execute
await execute({
  variables: { id: 1 },
  context: { headers: { "X-Custom": "value" } },
});

// To change fetchPolicy, rerender the component
```

### 6. useMutation ignoreResults Removal

**What changed:** The `ignoreResults` option has been removed from `useMutation`.
...
```ts
// Apollo Client 4.0
// Use client.mutate directly if you don't need component state sync
...
### 7. Type Namespace Changes

**What changed:** Hook types have been moved into namespaces.

**Before:**

```ts
// Apollo Client 3.x
import { QueryResult, QueryHookOptions, MutationResult } from "@apollo/client";

const options: QueryHookOptions<TData, TVariables> = {
  /* ... */
};
const processResult = (result: QueryResult<TData>) => {
  /* ... */
};
```
...
const options: useQuery.Options<TData, TVariables> = {
  /* ... */
};
const processResult = (result: useQuery.Result<TData>) => {
  /* ... */
};
```

## Priority 4: TypeScript and API Changes

### 8. Generic Parameter Removals

**What changed:** `TCacheShape` and `TSerialized` generics removed.

**Before:**

```ts
// Apollo Client 3.x
const client = new ApolloClient<NormalizedCacheObject, SerializedType>({
  cache: new InMemoryCache(),
});

class CustomCache extends ApolloCache<SerializedType> {
  extract(): SerializedType {
    /* ... */
  }
}
```
...
// client.extract() returns unknown
const extracted = client.extract() as MySerializedType;

class CustomCache extends ApolloCache {
  extract(): MySerializedType {
    /* ... */
  }
}
```

### 9. TContext Replaced with DefaultContext

**What changed:** `TContext` generic replaced with declaration merging.
...
const link = new ApolloLink((operation, forward) => {
  const context = operation.getContext() as MyContext;
  // ...
});
```
...
// Context is now properly typed everywhere
const link = new ApolloLink((operation, forward) => {
  const context = operation.getContext(); // typed as DefaultContext
  console.log(context.userId); // properly typed
});
```

## Priority 5: Link and Network Changes

### 10. Link Classes vs Functions

**What changed:** Link creator functions have been replaced with classes, and static methods no longer accept plain functions.

⚠️ **Important:** `SetContextLink` flips the callback arguments compared to `setContext`!
...
import { setContext } from "@apollo/client/link/context";
import { onError } from "@apollo/client/link/error";
import { concat, split, from } from "@apollo/client";
...
// Could pass plain functions to concat/split
const link = concat((operation, forward) => forward(operation), httpLink);
```
...
// Note: SetContextLink flips the arguments - prevContext comes first, operation second
...
const errorLink = new ErrorLink(({ error }) => {
  // error handling with unified error
});

// Must wrap functions in ApolloLink
const link = ApolloLink.concat(
  new ApolloLink((operation, forward) => forward(operation)),
  httpLink
);

// Use operationType for cleaner conditions
...
### 11. Observable and Query Lifetime Changes

**What changed:** ObservableQuery behavior with subscriptions and retain.

**Before:**

```ts
// Apollo Client 3.x
// Unsubscribing would terminate the network request
const subscription = observableQuery.subscribe({
  next: (result) => {
    /* ... */
  },
});
subscription.unsubscribe(); // Stops the network request
```
...
```ts
// Apollo Client 4.0
// Use retain() to keep network request running
const promise = observableQuery.refetch();
promise.retain(); // Keep running even if unsubscribed

// Or for regular subscriptions - the request continues by default
const subscription = observableQuery.subscribe({
  next: (result) => {
    /* ... */
  },
});
subscription.unsubscribe(); // Request continues unless no other subscribers
```

## Priority 6: Testing Utilities

### 12. MockLink Default Delays

**What changed:** MockLink now has default delays to ensure tests handle loading states.
...
const mocks = [
  {
    ...
    // No delay by default - immediate response
  },
];
```
...
// Default uses realisticDelay() (20-50ms)
const mocks = [
  {
    request: { query: MY_QUERY },
    result: {
...
// To restore old behavior globally:
...
### 13. Testing Utility Removals

**What changed:** Several testing utilities have been removed.

**Before:**

```ts
// Apollo Client 3.x
import { createMockClient, mockSingleLink } from "@apollo/client/testing";
...
## Priority 7: Cache Implementation Changes

### 14. Custom Cache Requirements

**What changed:** Custom caches must implement `fragmentMatches`.
...
### 15. canonizeResults Removal

**What changed:** The `canonizeResults` option has been removed.
...
const cache = new InMemoryCache({
  canonizeResults: true, // Prone to memory leaks
});
```
...
// Apollo Client 4.0
const cache = new InMemoryCache({
  // canonizeResults option no longer exists
  // Some referential equality may be lost
});
```

## Priority 8: SSR Changes

### 16. SSR Method Updates

**What changed:** New `prerenderStatic` method replaces deprecated SSR methods.
...
const { html, data } = await prerenderStatic(<App />, {
  // Works with React 19 prerender APIs
});
```

### 17. prioritizeCacheValues Renaming

**What changed:** `disableNetworkFetches` renamed to `prioritizeCacheValues`.
...
// Queries would be permanently changed to cache-first
```
...
```ts
// Apollo Client 4.0
client.prioritizeCacheValues();
// Queries keep original fetchPolicy, just prioritize cache temporarily
```

## Priority 9: Other Notable Changes

### 18. notifyOnNetworkStatusChange Default

**What changed:** `notifyOnNetworkStatusChange` now defaults to `true`.

**Before:**

```ts
// Apollo Client 3.x
// No loading states on refetch by default
const { data, loading, refetch } = useQuery(QUERY);
await refetch(); // loading stays false
```
...
```ts
// Apollo Client 4.0
// Loading states on refetch by default
const { data, loading, refetch } = useQuery(QUERY);
await refetch(); // loading becomes true
...
### 19. ObservableQuery Registration

**What changed:** ObservableQueries only registered while they have subscribers.
...
// Immediately registered with client
client.getObservableQueries(); // Map includes this query
```
...
const observableQuery = client.watchQuery({ query });
// Not registered yet
client.getObservableQueries(); // Set is empty

const sub = observableQuery.subscribe({ next: () => {} });
client.getObservableQueries(); // Set now includes this query

sub.unsubscribe();
client.getObservableQueries(); // Set is empty again (unless other subscribers)
```

## Notes on Documentation Approach

1. **Focus on practical migration:** Each section should show concrete before/after code examples
2. **Group related changes:** Keep related changes together (e.g., all error handling changes)
3. **Prioritize by impact:** Most common/impactful changes should come first
4. **Link to detailed docs:** Migration guide should link to full documentation for complex topics
5. **Avoid duplicating existing sections:** Build upon what's already documented, don't repeat

## Items to Skip/Minimize

Based on the review, these items don't need extensive documentation in the migration guide:

- Minor type name changes (FetchResult → ApolloLink.Result, etc.) unless affecting common usage
- Bug fixes and implementation details
- Features that were long-deprecated (3+ years)
- Internal/experimental APIs that were in `/internal` paths
- New features that don't affect migration (dataState, DataValue namespaces)
- Implementation details (RenderPromises removal, parser cache removal)
