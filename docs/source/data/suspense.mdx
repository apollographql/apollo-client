...
When the `GET_DOG_QUERY` inside of the `Dog` component returns a GraphQL error or a network error, `useSuspenseQuery` throws and the nearest error boundary renders its fallback component.
...
Here, we're using our `ErrorBoundary` component and placing it **outside** of our `Dog` component. Now, when the `useSuspenseQuery` hook inside the `Dog` component throws an error **when using the default `errorPolicy` of `none`**, the `ErrorBoundary` will catch it and display the `FallbackComponent` we've provided.
