...
Apollo Client has built-in support for communicating with a GraphQL server over HTTP via it's `HttpLink`.
To set up this communication, provide a `HttpLink` instance as `link` parameter to the `ApolloClient` constructor:
...
You can specify the names and values of custom headers to include in every HTTP request to a GraphQL server. To do so, provide the `headers` parameter to the  `HttpLink` constructor, like so:
...
<InterfaceDetails headingLevel={3} canonicalReference="@apollo/client!HttpLink.HttpOptions:interface" displayName='HttpLink.HttpOptions' />
