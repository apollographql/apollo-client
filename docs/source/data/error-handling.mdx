...
These are errors encountered while attemptint to communicate with your GraphQL server, usually resulting in a `4xx` or `5xx` response status code (and no data).
...
You can add retry logic and other advanced network error handling to your application with [Apollo Link](#handling-network-errors-with-apollo-link).
...
To retry operations that encounter a network error, we recommend adding a `RetryLink` to your link chain. This link enables you to configure retry logic like backoff and number of attempts.
