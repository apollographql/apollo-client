...
> Apollo Client 3.0 is now available in beta. [Switch to the beta docs](/react/v3.0-beta)
...
> You can also start and stop polling dynamically with the `startPolling` and `stopPolling` functions that are returned by the [`useQuery` hook](/react/api/react-hooks/#usequery).
