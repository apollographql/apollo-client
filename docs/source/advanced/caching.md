...
**Note**: The cache you declared above using `InMemoryCache` class is not used directly. It is passed to client created using `ApolloClient`. client is then used to access the cache using readQuery/writeQuery. There's a difference between `cache.writeQuery` and `client.writeQuery`. client uses cache internally for writeQuery operations, however, it does a broadcast using `client.initQueryManager().broadcastQueries()` after the cache has been written. This ensures that the data is refreshed in the view after the writeQuery operation. If you only use `cache.writeQuery` then the cache gets updated. But the changes will not reflect in your view. This is sometimes useful for scenarios where you want to write to the cache and read it later.
