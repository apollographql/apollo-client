...
### Using `merge: false` to disable `merge`-related warnings

You might encounter console warnings like "Cache data may be lost when...", even when the default behavior (incoming data replacing existing data) is desirable. In this case, you can disable the warning by passing `merge: false` to a field's `FieldPolicy` like so:
