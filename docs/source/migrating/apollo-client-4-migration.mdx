...
Certain default options might affect the return types of methods like `client.query` and hooks like `useQuery` or `useSuspenseQuery`. When you set `errorPolicy` or `returnPartialData` in your `ApolloClient` `defaultOptions`, declare them at the type level so TypeScript can accurately type your results.

#### What changed

When you set certain default options in Apollo Client 3, the TypeScript types for query results didn't reflect those defaults. For example:
...
Declaring these options ensures you provide values for them in your `defaultOptions`.
If you don't declare these options, you can't use these properties in the `defaultOptions` you pass to the `ApolloClient` constructor (other `defaultOptions` are still allowed).

#### Opting out

If you create multiple Apollo Client instances with conflicting default options and can't declare a single global default, you can opt out by declaring the options as `unknown`:
