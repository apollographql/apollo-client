...
In some cases, you may want to render partial data alongside an error. To do this, you can set the `errorPolicy` option to `all`. `useSuspenseQuery` will avoid throwing the error and instead set an `error` property returned by the hook. To ignore errors altogether, set the `errorPolicy` to `ignore`.
