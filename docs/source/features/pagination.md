...
In Apollo, the easiest way to do pagination is with a function called [`fetchMore`](/advanced/caching#fetchMore), which is included in the properties provided by the `Query` component. This basically allows you to do a new GraphQL query and merge the result into the original result.
