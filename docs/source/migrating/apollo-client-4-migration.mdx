...
The `operationName` is now set to `undefined` when executing an anonymous query. Anonymous queries were previously was set as an empty string.
...
Use the `client` property to get access to the `cache.identify` function to identity an object.
