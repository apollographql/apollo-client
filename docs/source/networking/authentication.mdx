...
Apollo Client uses the ultra flexible [Apollo Link](/react/api/link/introduction/) that includes several options for authentication.
...
In this particular example headers are set individually for every request but you can also use `headers` option. See the Apollo Boost [Configuration options](../get-started/#configuration-options) section for more details.
