...
> **Note:** in Apollo Client >= v3.8, data fetching hooks `useSuspenseQuery` and `useBackgroundQuery` are available for querying data within `<Suspense />` boundaries using React 18's new concurrent rendering model. For more information see Apollo Client's [Suspense docs](../react/suspense/).
