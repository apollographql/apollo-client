...
A GraphQL server provides selective access to a sprawling graph of data that GraphQL clients might request, containing countless entity objects and their fields. A typical GraphQL client needs only a small subgraph of this complete data graph, and uses GraphQL queries to fetch all and only the fields it needs.
...
Rather than attempting to standardize on one style of pagination to serve all use cases, Apollo Client enables any and all such patterns through the highly flexible `InMemoryCache` field policy API, which  allows you to configure the client-side behavior of fields across your graph in a consistent, declarative, reusable way, in one place, rather than reproducing custom field logic wherever the field is used across your application. You can get started by copying and pasting example code from this documentation, but you will no doubt end up adapting and expanding your field policies to suit your specific needs. Whenever you need to apply similar logic elsewhere, remember that you can encapsulate common policies in reusable policy-generating functions.
...
Without input from you, `InMemoryCache` has no way of knowing how to interpret these field arguments, so it must assume they are all important. In other words, if you write two consecutive pages into a cache that does not know about your pagination system, like so:
```ts
...
```
the cache by default stores those two results separately, using keys derived from both the field name (`feed`) and the serialized arguments (`limit`, `offset`, and `type`):
```js
...
```
However, this simple strategy makes some risky assumptions about the order of the written pages, because it ignores the `offset` and `limit` arguments. In order to handle those arguments correctly, your `merge` function can use `options.args` to decide where to put the incoming data:
```js
const cache = new InMemoryCache({
  typePolicies: {
    Query: {
