...
- Retrieving `@client` data with the help of read functions (step 1 above) is explained in [Querying a local-only field](./managing-state-with-field-policies#querying-a-local-only-field).
