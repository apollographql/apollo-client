...
This same error handling behavior applies to **subscriptions** when using `client.subscribe()`. All errors are delivered via the `next` callback in `result.error`, never via the `error` callback. The subscription only terminates (successfully) after emitting non-recoverable errors like network failures via `next`. See the [subscriptions documentation](./subscriptions/#handling-errors-in-subscriptions) for more details.
