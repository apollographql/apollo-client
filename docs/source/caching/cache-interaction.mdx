...
When you need to perform multiple cache operations together, `cache.batch` lets you group them into a single transaction. This provides two key benefits:

1. **Performance**: Watchers (and thereby React components) are notified only once after all operations complete, rather than after each individual operation.
2. **Control**: You gain fine-grained control over optimistic updates and can intercept watcher notifications.
...
| Value | Behavior |
| --- | --- |
| `string` | Creates a new optimistic layer with the given ID. Use this when you want to later remove these specific changes. |
...
cache.batch({
  ...
  onWatchUpdated(watch, diff, lastDiff) {
    ...
    // Return false to prevent this watcher from being notified
