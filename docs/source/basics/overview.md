---
title: Apollo Client Overview
subtitle: Introducing GraphQL with Apollo Client
description: An overview of what Apollo Client does and how it works
---

<h2 title="topic">Project Goals</h2>
Apollo Client is a fully-featured caching GraphQL client with integrations for React, Angular, and more. It allows you to easily build UI components that fetch data via GraphQL. To get the most value out of Apollo Client, you should use it with one of its view layer integrations. To get started with the React integration, go to [setup](LINK PLZ).

Apollo Client also has view layer integrations for [all the popular frontend frameworks](LINK PLZ). For the best experience, make sure to use the view integration layer for your frontend framework of choice.
...
<h3 title="docs-overview">Documentation Overview</h3>
The docs for Apollo Client are mainly written using the [React integration](LINK PLZ) but things that are only supported by that integration are marked as *React only*. The docs are broken into four distinct sections:

1. **Basics**, which outline the why and what of using Apollo Client to build your application.
2. **Features**, which showcase some of the advanced capabilities of Apollo Client that your app may need.
3. **Recipes**, to isolate and explain how to do common patterns that you may need.
4. **Reference**, to act as the index to find API details for the client.
