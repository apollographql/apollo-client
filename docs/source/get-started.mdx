...
> for your React app. For a more complete introduction to the entire Apollo platform, [check out **Odyssey**, Apollo's new learning platform](https://odyssey.apollographql.com?utm_source=apollo_docs&utm_medium=referral).
...
If you're an advanced user who would like to configure Apollo Client from scratch, head on over to our [Apollo Boost migration guide](./migrating/boost-migration/). For the majority of users, Apollo Boost should meet your needs, so we don't recommend switching unless you absolutely need more customization.
...
| `clientState`    | { resolvers?: Object, defaults?: Object, typeDefs?: string \| Array &lt;string&gt; }                                         | An object representing your local state configuration. This is useful if you would like to use the Apollo cache for [local state management](./data/local-state/).                                                                                                                                                                           |
| `cacheRedirects` | Object                                                                                                                       | A map of functions to redirect a query to another entry in the cache before a request takes place. This is useful if you have a list of items and want to use the data from the list query on a detail page where you're querying an individual item. More on that [here](./performance/performance). |
| `credentials`    | string                                                                                                                       | Is set to `same-origin` by default. This option can be used to indicate whether the user agent should send cookies with requests. See [Request.credentials](https://developer.mozilla.org/en-US/docs/Web/API/Request/credentials) for more details.                                                                                    |
| `headers`        | Object                                                                                                                       | Header key/value pairs to pass along with the request.                                                                                                                                                                                                                                                                                            |
| `fetch`          | WindowOrWorkerGlobalScope['fetch']                                                                                                         | A [`fetch`](https://developer.mozilla.org/en-US/docs/Web/API/Fetch_API) compatible API for making a request.                                                                                                                                                                                                                           |
