...
In certain cases, writing an `update` function to [update the cache after a mutation](./mutations/#updating-the-cache-after-a-mutation) can be complex, or even impossible if the mutation doesn't return modified fields.
