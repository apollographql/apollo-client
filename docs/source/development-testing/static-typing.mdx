...
In Apollo Client, all APIs that accept a `DocumentNode` can alternatively take `TypedDocumentNode<Data, Variables>`. This type enables Apollo Client APIs to infer the data and variable types using the GraphQL document.
...
We recommend using `TypedDocumentNode` node and rely on type inference instead of specifying the generic type arguments on Apollo Client APIs such as `useQuery`. This makes GraphQL documents more portable and provides better type safety wherever the document is used.
...
// ✅ Add the type to the query using `TypedDocumentNode` instead
