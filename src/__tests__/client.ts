...
describe("data masking", () => {
  ...
  it("does not trigger update when updating field in named fragment", async () => {
    interface Query {
      currentUser: {
        __typename: "User";
        id: number;
  ...
  it("warns when accessing a fragmented field while using @unmask", async () => {
...

    {
      ...
      expect(consoleSpy.warn).toHaveBeenCalledWith(
        "Field '%s' will not be available when masking query %s. Please read the field from the fragment instead.",
        "age"
        // "'UnmaskedQuery'"
      );
