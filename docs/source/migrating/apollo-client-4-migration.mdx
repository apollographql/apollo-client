...
### Errors never terminate `ObservableQuery` or subscription Observables
...
Apollo Client 4 changes this and no longer emits an `error` notification. This ensures observables don't terminate on errors and can continue listening for new results, such as calling `refetch` after an error.
...
This change applies to **all** Apollo Client observables, including subscriptions created with `client.subscribe()`. GraphQL errors in subscriptions are always delivered via the `next` callback in `result.error`, never via the `error` callback. The subscription only terminates for non-recoverable errors like network failures.
