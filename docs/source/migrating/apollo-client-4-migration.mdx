...
Apollo Client 4 removes `ApolloError` entirely. This change has several implications.

#### External errors are no longer wrapped

External errors (i.e. network errors) are no longer wrapped and are instead returned as-is. This makes it easier to debug the source of the error as the stack trace now points to the location of the error.

If you previously handled network errors by checking the `error.networkError` property, use the `error` property directly.
...
Apollo Client 4 guarantees that the `error` property is an `ErrorLike` object, an object with a `message` and `name` property, as a replacement for `ApolloError`. Thrown values that are non-error-like objects (such as a symbol or primitive value) are wrapped in an `UnconventionalError` error instance to avoid the need to check the type of the error before consuming it. `UnconventionalError` sets its `name` to `UnconventionalError`, sets its `message` to `"An error of unexpected shape occured."`, and sets its [`cause` property](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Error/cause) to the value originally thrown.

#### X

Apollo Client also provides more granular error classes for errors thrown by the client.

For a more comprehensive look at all of the available error classes and how to check for specific kinds of errors, read the [Error handling](../data/error-handling) guide.

### Network errors now adhere to the `errorPolicy`
