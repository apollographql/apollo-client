...
If you have previously used `getInitialState` for SSR, that API has been moved to the cache itself instead of on the client. You can read more about the recipe for server side rendering [here](/v2.4/features/server-side-rendering/). The upgrade path looks like this:
...
Query reducers have been finally removed in the 2.0, instead we recommend using the more flexible [`update`](/v2.4/essentials/mutations/) API instead of reducer.
