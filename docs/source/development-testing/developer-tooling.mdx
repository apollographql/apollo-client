...
While your app is in dev mode, the Apollo Client Devtools will appear as an "Apollo" tab in your web browser inspector. To enable the devtools in your app in production, pass `devtools: { enabled: true }` to the `ApolloClient` constructor in your app. Pass `devtools: { enabled: false }` if you want to manually disable this functionality.
