...
This same error handling behavior applies to subscriptions when using `client.subscribe()`. All errors are delivered via the `next` callback in `result.error`, never via the `error` callback. When an error causes the connection to terminate, the error is emitted via `next`, followed by a `complete` notification. See the [subscriptions documentation](./subscriptions/#handling-errors-in-subscriptions) for more details.
