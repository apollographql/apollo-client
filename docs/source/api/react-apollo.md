...
An important note is that `compose()` executes the first enhancer _first_ and works its way forwards through the list of enhancers. To illustrate calling three functions like this: `funcA(funcB(funcC(component)))` is equivalent to calling `compose()` like this: `compose(funcC, funcB, funcA)(component)`.
