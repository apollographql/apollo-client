...
**Most importantly, modifying a reactive variable triggers an update of every active query that depends on that variable, as well an update of the react state associated with all instances returned from the useReactiveVar React hook.** A query depends on a reactive variable if any of the query's requested fields defines a [`read` function](../caching/cache-field-behavior/#the-read-function) that reads the variable's value.
