...
Here's our cache redirect, which we can set up easily by specifying a map on the `cacheRedirects` property of `ApolloClient`. The cache redirect returns a key that the query can use to look up the data in the cache.
