...
<FunctionDetails canonicalReference="@apollo/client/react!useQuery:function(1)" headingLevel={2}>
  ...
  The query result includes a new `dataState` property that indicates the completeness of the data:
  ...
  if (dataState === "partial") {
    // data is DeepPartial<TData>
  }

  if (dataState === "streaming") {
    // data is TData but still streaming
  }

  if (dataState === "complete") {
    // data is complete TData
  }
  ```

  The hook also maintains previous data when emitting a loading state, unless the query or variables have changed. This means you can safely use the previous data while loading new data.

  Note that `@exports` variables are not taken into account for this behavior, so data will stay around even if they change.
</FunctionDetails>
...
<FunctionDetails canonicalReference="@apollo/client/react!useLazyQuery:function(1)" headingLevel={2}>
  ...
  <ManualTuple>
    ...
    >
      ...
      If the `execute` function is called while a previous query from the same `useLazyQuery` usage is still in flight, the previous query will be rejected with an `AbortError` unless `.retain()` is called on the promise returned by the previous `execute` call.

      Please keep in mind that `useLazyQuery` is primarily meant as a means to synchronize your component to the status of a query and that its purpose is not to make a series of network calls. If you plan on making a series of network calls without the need to synchronize the result with your component, consider using `ApolloClient.query` instead.
    </ManualTupleItem>
    <ManualTupleItem
      name="result"
