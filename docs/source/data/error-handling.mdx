...
When errors occur, Apollo Client returns them in the `error` field of the result object returned by hooks like `useQuery`, `useMutation`, and `useSubscription`. The `error` field contains an `ErrorLike` object that provides details about what went wrong.
...
Apollo Client then adds those errors to the `error` field returned by your `useQuery` call (or whichever operation hook you used), wrapped as a `CombinedGraphQLErrors` object.
...
When a network error occurs, Apollo Client adds it to the `error` field returned by your `useQuery` call (or whichever operation hook you used) as an `ErrorLike` object.
