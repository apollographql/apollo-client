...
While the response is streaming, the `loading` flag remains `true` to indicate that the query has not yet fully completed. The `networkStatus` field will start as `NetworkStatus.loading` and change to `NetworkStatus.streaming` as the initial chunk arrives. Once all deferred data has been received, `loading` will become `false` and `networkStatus` will be `NetworkStatus.ready`.

When subsequent chunks of deferred data arrive, `useQuery` will re-render and `data` will include the deferred data as they arrive.
...
### Refetch without changing variables
...
## Setting up `@defer` support by choosing an `incrementalHandler`

As the transport format for `@defer` has not been standardized yet and there have been multiple proposals over time, Apollo Client does not default to a specific format.
Instead, you need to configure the incremental delivery format handler you want to use.
