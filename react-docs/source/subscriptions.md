...
Note that the `updateQuery` callback must return an object of the same shape as the initial query data otherwise the new data won't be merge. Here the new comment is pushed in the `comments` list of the `entry`:
