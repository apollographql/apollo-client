...
function AddTodo() {
  let input;
  const [addTodo, { data }] = useMutation(ADD_TODO, FRAGMENT, 'AddTodo', 'todos');
...
> Refer to the [Mutations](../../data/mutations/) section for a more in-depth overview of `useCachedMutation`.
