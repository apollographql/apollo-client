...
- Retrieving `@client` data with the help of read functions (step 1 above) is explained in [Define a field policy](./managing-state-with-field-policies/#define-a-field-policy).
- Loading `@client` data from the cache (step 2 above) is explained in [Storing local state in the cache](./managing-state-with-field-policies#storing-local-state-in-the-cache).
...
- [`cache.modify`](../caching/cache-interaction/#cachemodify
