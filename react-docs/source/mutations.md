...
Note the use of the [`name` option on the `grapql()` call](higher-order-components.html#graphql-api) to name the prop that will receive the mutation function for each mutation (by defaut that name is 'mutate').
