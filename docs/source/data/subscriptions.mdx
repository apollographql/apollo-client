...
The following example expands on the one in the previous step by initializing both a `WebSocketLink` _and_ an `HttpLink`. It then uses the `split` function to combine those two `Link`s into a _single_ link that uses one or the other according to the type of operation being executed.
...
//
// * A function that must return a boolean value
// * The Link to use if the function returns true
// * The Link to use if the function returns false
...
With this code, queries and mutations will use HTTP as normal, and subscriptions will use WebSocket.
