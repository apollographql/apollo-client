...
## Defining default options for type safety
...
Define your default options using TypeScript's [declaration merging](https://www.typescriptlang.org/docs/handbook/declaration-merging.html) with the `ApolloClient.DeclareDefaultOptions` namespace. Create a TypeScript declaration file in your project:
...
If you can't define specific global defaults (for example, if you have multiple Apollo Client instances with different default options), you can opt out by defining the options as `unknown`:
