...
import { LocalState, FragmentMatcher } from "./LocalState";
...
export class ApolloClient<TCacheShape> implements DataProxy {
  ...
  constructor(options: ApolloClientOptions<TCacheShape>) {
     ...
     */
   if (!hasSuggestedDevtools && __DEV__ && connectToDevTools) {
  ...
  >(
    ...
    result.catch((error) => {
      invariant.debug(
        `In client.refetchQueries, Promise.all promise rejected with error ${error}`
      );
    });
