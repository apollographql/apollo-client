...
Now that we've [learned how to fetch data](/essentials/queries/) from our backend with Apollo Client,
...
object (fully documented in the [API reference](/api/react-hooks/#result-2)) include booleans that indicate whether the mutate function has been `called` yet, and whether the mutation's result is currently `loading`.

## Updating the cache after a mutation

When you execute a mutation, you modify back-end data. If that data
is also present in your [Apollo Client cache](/advanced/caching/),
...
Learn about all of the fields returned by `useMutation` in the [API reference](/api/react-hooks/#result-2).
...
detail with usage examples, see the [API reference](/api/react-hooks/#usemutation).
