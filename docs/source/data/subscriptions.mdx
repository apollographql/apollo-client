...
In Apollo Client 4, error handling for subscriptions works differently than you might expect. GraphQL errors are **always delivered via the `next` callback** in the subscription result, never via the `error` callback.
...
When using `client.subscribe()` directly, GraphQL errors are delivered via the `next` callback in `result.error`, not via the `error` callback:

```ts
client.subscribe({ query: MY_SUBSCRIPTION }).subscribe({
  next: (result) => {
    if (result.error) {
      // GraphQL errors are delivered here
  ...
  error: (error) => {
    // This callback is never called for GraphQL errors.
    // Only non-recoverable errors like network failures
    // will cause the subscription to complete and be reported here.
  },
});
```

This behavior ensures that subscriptions can continue running even after encountering errors, allowing your server to send valid data after an error occurs. The subscription only terminates for non-recoverable errors like network failures.
