...
Supported values are listed in [Options](#options).
</td>
</tr>
...
Supported values are listed in [Options](#options-3).
</td>
</tr>
...
For usage instructions, see [Interacting with cached data: Identify cached entities](../../caching/cache-interaction/#obtaining-an-objects-custom-id).
...
Retruns a serialized representation of the cache's current contents as a `NormalizedCacheObject`.
