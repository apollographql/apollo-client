...
2a. Reading `@client` data from the cache (step A above) is explained in [2a. Handling `@client` fields with the cache](#2a-handling-client-fields-with-the-cache).
2b. Resolving `@client` data with the help of local resolvers (step B above) is explained in [2b. Handling `@client` fields with resolvers][].
