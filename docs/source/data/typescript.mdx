...
result.data; // Actual runtime type: TData | undefined
```
...
If you can't declare specific global defaults (for example, if you have multiple Apollo Client instances with different default options), you can opt out by declaring those options as optional with a union type, or as `unknown`:
