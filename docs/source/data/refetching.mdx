...
`cache-only` queries are excluded from `refetchQueries` in all situations, including when affected by `updateCache` operations (unless `onQueryUpdated` is provided).

To refetch _all_ queries managed by Apollo Client (even those with a `fetchPolicy` of `"standby"`), pass `"all"` for `include`:
