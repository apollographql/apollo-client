...
> **Important**: To use `@client` fields, you must configure your `ApolloClient` with a `LocalState` instance. See the [local state documentation](../local-state/local-state-management/) for setup instructions.
