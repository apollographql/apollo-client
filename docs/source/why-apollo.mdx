...
Once you've passed your cache to `ApolloClient`, whenever Apollo Client receives query response data, it will automatically attempt to identify and store the distinct objects (i.e., those with a `__typename` and an id property) from a query's data into separate entries within its cache.
...
If your company uses Apollo Client in production, we'd love to feature a case study on our blog! Please get in touch via [our community forums](https://community.apollographql.com) so we can learn more about how you're using Apollo. Alternatively, please send in a PR if you already have a blog post or a conference talk that you'd like to feature here.
