...
> **Note:** The `WatchQuery` interface applies to React hooks (`useQuery`, `useSuspenseQuery`, `useBackgroundQuery`, `useLoadableQuery`, `useLazyQuery`). The `Query` interface applies to `client.query()` and the `Mutate` interface applies to `client.mutate()`. These are separate and do not affect each other.
...
declare module "@apollo/client" {
  namespace ApolloClient {
    namespace DeclareDefaultOptions {
      // Affects React hooks (useQuery, useSuspenseQuery, etc.)
...
If you create multiple Apollo Client instances with conflicting default options and can't declare a single global default, you can opt out by declaring those options as optional with a union type, or as `unknown`:
...
Doing that allows you to call the `ApolloClient` constructor with any arbitrary `defaultOptions`.
In that case, all methods and hooks return the broadest possible types for those options—for example, `data` is always typed as potentially `undefined` and partial data states are always included. This is because TypeScript can no longer narrow the return type based on your defaults.
