...
If you are using TypeScript, `data` is typed as `unknown` when the query type is unknown. Accessing fields on `data` results in a TypeScript error. See the guide on [Using TypeScript](../development-testing/static-typing) to learn how to add types for your queries.
