...
test("rerenders if changing variables returns same result for different variables with notifyOnNetworkStatusChange: false", async () => {
  ...
  // Discuss in PR: Determine if this makes sense. This render is from the rerender
  // above, but we see a loading state because `observable.getCurrentResult()`
  // returns the loading state immediately even though
  // `notifyOnNetworkStatusChange` is `false`. I think this is ok because
  // `notifyOnNetworkStatusChange` doesn't guarantee you won't see render
  // states, just that we won't emit additional renders specifically when
