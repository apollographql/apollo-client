...
| `refetchQueries`      | Array&lt;string\|{ query: DocumentNode, variables?: TVariables}&gt; \| ((mutationResult: FetchResult) => Array&lt;string\|{ query: DocumentNode, variables?: TVariables}&gt;) | A function that returns an array, or an array that allows you to specify which queries you want to refetch after a mutation has occurred. Array values can either be queries (with optional variables) or just names of queries to be refeteched.                               |
