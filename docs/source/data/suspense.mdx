...
In this guide, we'll look at Apollo Client's new data fetching hooks introduced in 3.8—specifically `useSuspenseQuery`, `useBackgroundQuery` and `useReadQuery`—which leverage React's powerful concurrent features and walk through some examples of how they can be used.
