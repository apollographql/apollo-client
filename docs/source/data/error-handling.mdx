...
Apollo Client then adds those errors to the `error` field returned by the `useQuery` hook (or whichever operation hook you used), wrapped as a [`CombinedGraphQLErrors`](#combinedgraphqlerrors) error instance.
...
Learn more about the available errors in the ["Error classes"](#error-classes) section.
...
All error classes provide a static `is()` method to determine if an error is
that type of error. The `is()` method type narrows the error to allow access on
additional properties for each error type in a type-safe way:
...
Apollo Client provides several built-in error classes to help you identify and handle different types of errors:
...
Fatal transport-level errors returned when executing a subscription using the multipart HTTP subscription protocol. These errors indicate issues with the subscription transport mechanism.

```ts
import { CombinedProtocolErrors } from "@apollo/client/errors";

if (CombinedProtocolErrors.is(error)) {
  // Handle protocol-level subscription errors
  error.errors.forEach((protocolError) => {
    console.log(protocolError.message, protocolError.extensions);
  });
}
```
...
Thrown when a non-200 response is returned from the server. This error contains the full server response, including status code and body text.
...
Thrown when failing to parse the response as JSON from the server. This error helps debug issues where the server returns malformed JSON or non-JSON content.
...
Represents an error when executing `@client` fields from `LocalState`, typically to indicate a problem with the `LocalState` configuration. This error does not represent an error thrown in a local resolver.

```ts
import { LocalStateError } from "@apollo/client/errors";

if (LocalStateError.is(error)) {
  console.log(`Local state error at path: ${error.path}`);
...
A wrapper error type that represents a non-standard error thrown from a response, such as a symbol or plain object. This ensures all thrown values can be handled as an `ErrorLike` instance. Read the `cause` property to determine the original source of the error.
