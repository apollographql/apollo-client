...
To avoid TypeScript errors, you must register the event with the `RefetchEvents` interface. See the [declaring refetch event types](./typescript#declaring-refetch-event-types) guide to learn more.
...
To avoid TypeScript errors, a non-`void` value type must be registered with the event declared in the `RefetchEvents` interface. See the [declaring refetch event types](./typescript#declaring-refetch-event-types) guide to learn more.
...
To avoid TypeScript errors, you still need to register sourceless events with TypeScript so that `emit` calls and `refetchOn` keys stay type-checked. See the [declaring refetch event types](./typescript#declaring-refetch-event-types) guide for how to type sourceless events and `void` event payloads.
...
See [Declaring refetch event types](./typescript#declaring-refetch-event-types) for how to register custom events on Apollo Client's `RefetchEvents` interface so `emit`, source functions, and `refetchOn` callbacks all stay type-safe. See the [refetching guide](./refetching) to learn more about the `client.refetchQueries` API.
