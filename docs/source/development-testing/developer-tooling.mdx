...
The Apollo Client AI skill is an AI-powered assistant that can help you with questions about Apollo Client setup, configuration, troubleshooting, and best practices. Install it with:

```bash
npx skills add apollographql/skills --skill apollo-client
```

The AI skill provides contextual help tailored to Apollo Client 4.x, including guidance on hooks, caching strategies, and React integration patterns. It's available when using AI assistants that support skills.
