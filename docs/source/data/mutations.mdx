...
* An object with a `query` field that specifies the query to execute, along with a `variables` field if applicable
* The name of a query you've previously executed, as a string (e.g., `GetComments`)
  ...
  * Queries you refer to by name are executed with their most recently provided set of variables.
...
const [addTodo, { data, loading, error }] = useMutation(ADD_TODO, {
  refetchQueries: [{
    ...
    {
      query: GET_POST, // Full query document wrapped in gql
