...
> You can use `connectionParams` for anything else you might need, not only authentication, and check its payload on the server side with [SubscriptionsServer](/apollo-server/data/subscriptions/#example-authentication-with-apollo-client).
