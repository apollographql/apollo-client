...
To avoid TypeScript errors, register a non-`void` value type with the event declared in the `RefetchEvents` interface. See the [TypeScript section](#typescript) to learn more.
...
You can customize the handler for an event by passing a function for an event in the `handlers` option. The function receives a context object that contains the `client` instance connected to it, the event `source` and `payload`, as well as a [`matchesRefetchOn`](#using-matchesrefetchon) helper function that provides the logic necessary to filter queries based on their `refetchOn` option.
...
Register custom events with TypeScript to provide full type safety for refetch events. Declare your custom events using TypeScript's [declaration merging](https://www.typescriptlang.org/docs/handbook/declaration-merging.html) with the `RefetchEvents` interface.
