...
class TestCache<TSerialized = any> extends ApolloCache<TSerialized> {
  public read<T, TVariables = any>(query: Cache.ReadOptions<TVariables>): T {
...
describe('abstract cache', () => {
  ...
  describe('readQuery', () => {
    ...
    it('defaults optimistic to false', () => {
      const test = new class extends TestCache {
        public read<T, TVariables = any>(
          query: Cache.ReadOptions<TVariables>,
        ): T | null {
          return query.optimistic as any;
        }
      }();
  ...
  describe('writeData', () => {
    ...
    it('reads __typename from typenameResult or defaults to __ClientData', () => {
      ...
      let res;
      test.writeFragment = options =>
        (res = (options.fragment.definitions[0] as FragmentDefinitionNode)
