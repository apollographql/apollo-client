...
Now that we've [learned how to fetch data](/data/queries/) from our backend with Apollo Client,
...
is also present in your [Apollo Client cache](/local-data/caching/),
...
> Learn more about supported cache functions in [Caching data](/local-data/caching/).
...
- [Local state](/local-data/local-state/): Use Apollo Client to manage the entirety of your application's local state by executing client-side mutations.
- [Caching in Apollo](/local-data/caching/): Dive deep into the Apollo Client cache and how it's normalized. Understanding the cache is helpful when writing update functions for your mutations!
