...
To use the `@stream` directive, apply it to a list field with an `initialCount` argument that specifies how many items to include in the initial response:
...
With this query, the server returns the first 5 items as soon as they're ready, then delivers the remaining items one at a time as they become available. This allows your UI to render part of the list while waiting for the remaining items.
...
For more information on using `@stream`, including details on how `loading`, `networkStatus`, and `dataState` change as list items arrive, see the [Defer](./defer) guide.
