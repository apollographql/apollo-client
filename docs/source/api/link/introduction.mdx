...
The following example uses the RxJS [`map` operator](https://rxjs.dev/api/index/function/map) to modify the result before returning it back up the link chain.
...
The `ReportErrorLink` link is still considered a stateless link despite being implemented as a subclass because it does not maintain state between requests.
