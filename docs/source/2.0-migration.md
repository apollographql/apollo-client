...
const sameOriginLink = = new ApolloLink((operation, forward) => {
...
const httpLink = createHttpLink({ uri: 'http://api.example.com/' })
...
const client = new ApolloClient({
  link: ApolloLink.from([sameOriginLink, httpLink]),
...
Query reducers have been finally removed in the 2.0, instead we recommend using the more flexible [`update`](LINK PLZ) API instead of reducer.
