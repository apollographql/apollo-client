...
Pass a callback function to the `refetchOn` option to add conditional logic for any event that would cause a refetch of the query. It receives a context object with the `source` name and the event's `payload`.
...
You can customize the handler for an event by passing a function for an event in the `handlers` option. It is called with a context object that contains the `client` instance connected to it, the event `source` and `payload`, as well as a [`matchesRefetchOn`](#using-matchesrefetchon) helper function that provides the logic necessary to filter queries based on their `refetchOn` option.
