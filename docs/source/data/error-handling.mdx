...
- **Observable-based APIs** (e.g. `client.watchQuery`) - Errors are emitted through observer callbacks
...
<Note>
  Suspense hooks like `useSuspenseQuery` have different error handling behavior - they might throw errors to React error boundaries based on the configured `errorPolicy`, rather than returning them in the hook result. Learn more about error handling with Suspense hooks in the [Suspense documentation](./suspense#error-handling).
</Note>
