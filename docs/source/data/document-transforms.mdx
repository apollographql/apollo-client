...
To do so, let's get the field node's `selectionSet` property and loop over its `selections` property to determine if it includes a field named `id`. It's important to note that a `selectionSet` may contain `selections` of both fields and fragments. Our implementation needs to perform a check against the selection's `kind` property to only check fields and not fragments. If we find a match on a field named `id`, we can stop the traversal of the AST. 
