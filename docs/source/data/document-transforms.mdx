...
To do so, let's get the field node's `selectionSet` property and loop over its `selections` property to determine if it includes a field named `id`. It's important to note that a `selectionSet` may contain `selections` of both fields and fragments. Our implementation only needs to perform checks against a field and not a fragment, so we check against the selection node's `kind` property to filter to field nodes. If we find a match on a field named `id`, we can stop traversal of the AST. 
