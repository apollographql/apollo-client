...
> **Note:** The `WatchQuery` interface applies to React hooks (`useQuery`, `useSuspenseQuery`, `useBackgroundQuery`, `useLoadableQuery`, `useLazyQuery`) and `client.watchQuery()`. The `Query` interface applies to `client.query()` and the `Mutate` interface applies to `client.mutate()`. These are separate and do not affect each other.
...
declare module "@apollo/client" {
  namespace ApolloClient {
    namespace DeclareDefaultOptions {
      // Affects React hooks (useQuery, useSuspenseQuery, etc.) and client.watchQuery()
...
If you create multiple Apollo Client instances with conflicting default options and can't declare a single global default, you can opt out by declaring those options as optional union types covering all values you use:
