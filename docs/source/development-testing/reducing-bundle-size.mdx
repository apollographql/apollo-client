...
Apollo Client uses the `__DEV__` variable instead of `process.env.NODE_ENV` because the latter is not available in non-node environments. Apollo Client can be used from a CDN with an ESM import. Usage of the `process` variable within the library in this scenario would cause a crash in your application.
