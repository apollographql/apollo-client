...
One of GraphQL's biggest advantages over a traditional REST API is its support for [declarative data fetching](/intro/benefits/#declarative-and-efficient-data-fetching). Each component can (and should) query exactly the fields it requires to render, with no superfluous data sent over the network.
...
Then if you've configured [metrics reporting in Apollo Studio](/studio/setup-analytics/), Apollo Server includes your app's `name` and `version` in the operation traces it reports to Studio. This enables you to [segment metrics by client](/studio/client-awareness/).
