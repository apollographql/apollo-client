...
In addition to `variables`, you can also optionally provide an entirely different shape of `query` to execute, which can be useful when `fetchMore` needs to fetch only a single field, but the original query contained some other unrelated fields. By default, the original query's shape will be reused.
...
Without any special configuration, `InMemoryCache` has no understanding of what these arguments mean, so it stores a separate value for each unique combination of arguments. Concretely, if you examine the internal cache data for a field with arguments, you will typically see those arguments serialized and appended to the end of the field name string, allowing multiple distinct values to be stored at the same time for a single field within a single entity object.
...
This field policy uses `keyArgs: false` to _disable_ the argument-based field value separation described above, along with a **`merge` function**. Both of these configurations are necessary for handling pagination:

* `keyArgs` specifies which arguments (if any) should cause the cache to store a _separate_ field value for each unique combination of those arguments.
  * In this case, the cache shouldn't store a separate result based on either `offset` _or_ `limit`, so we disable the field key system by passing `false`. An empty list would also work (`keyArgs: []`), but `keyArgs: false` is more expressive, and results in a field key that is simply the name of the field (`feed` in this case).
...
You define a `read` function for a field within its [field policy](#defining-a-field-policy), alongside the `merge` function and `keyArgs`. If you define a `read` function for a field, the cache calls that function whenever you query the field, passing any existing data as the first argument. In the query response, the field will be populated with the `read` function's return value, instead of the existing data.
