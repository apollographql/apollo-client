...
Usually, the elements of the `refetchQueries` array are the string names of the queries you want to refetch (e.g., `"GetTodos"`). The array can also include `DocumentNode` objects parsed with the `gql` function:

```js
const [addTodo, { data, loading, error }] = useMutation(ADD_TODO, {
  refetchQueries: ["GetTodos", MY_OTHER_QUERY],
});
```

> To refer to queries by their string name, make sure each of your app's queries has a _unique_ name.
