...
The `useQuery` [React hook](https://react.dev/reference/react) is the primary API for executing queries in an Apollo application. To run a query within a React component, call `useQuery` and pass it a GraphQL query string. When your component renders, `useQuery` returns an object from Apollo Client that contains `loading`, `error`, `data`, and `dataState` properties you can use to render your UI.
...
function Dogs({ onDogSelected }) {
  const { loading, error, data, dataState } = useQuery(GET_DOGS);

  if (loading) return 'Loading...';
  if (error) return `Error! ${error.message}`;

  // Handle different data states
  if (dataState === 'empty') return 'No dogs found';
  if (dataState === 'partial') return 'Loading more dogs...';
  if (dataState === 'streaming') return 'Streaming dogs...';
...
The `dataState` property indicates the completeness of the `data` property. It can have the following values:

- `empty`: The query has completed but returned no data
- `partial`: The query has returned some data but is still loading more
- `streaming`: The query is streaming data (when using `@defer` or `@stream` directives)
- `complete`: The query has completed and returned all data

As our query executes and the values of `loading`, `error`, `data`, and `dataState` change, the `Dogs` component can intelligently render different UI elements according to the query's state:

- As long as `loading` is `true` (indicating the query is still in flight), the component presents a `Loading...` notice.
- When loading is `false` and there is no `error`, the query has completed. The component then checks the `dataState` to determine how to handle the data.
- Finally, when `dataState` is `complete`, the component renders a dropdown menu that's populated with the list of dog breeds returned by the server.
...
The `useQuery` hook's result object provides fine-grained information about the status of the query via the `networkStatus` property. In Apollo Client 4.0, `notifyOnNetworkStatusChange` now defaults to `true`, which means that loading states will be emitted (core API) or rendered (React) by default when calling `refetch`, `fetchMore`, etc.

If you want to maintain the old behavior where loading states are not emitted during refetches, you can set `notifyOnNetworkStatusChange` to `false` in `defaultOptions`:
...
Here's an example of using `networkStatus` to show different loading states:
...
function DogPhoto({ breed }) {
  const { loading, error, data, refetch, networkStatus } = useQuery(
    ...
    {
      variables: { breed },
      // notifyOnNetworkStatusChange is true by default in Apollo Client 4.0
    }
  );
...
In Apollo Client 4.0, network errors now adhere to the `errorPolicy` setting, just like GraphQL errors. This means that promise-based APIs, such as `client.query`, will resolve the promise with the `error` property set to the network error instead of rejecting the promise, even if `errorPolicy` is set to `ignore`.
...
if (error) {
  // This will now handle both GraphQL errors and network errors
  console.error('Error details:', error);
  // The error object will have a bodyText property containing the raw response body
  if (error.bodyText) {
    console.error('Raw response:', error.bodyText);
  }
}

// With errorPolicy: 'all', we can still render partial data
if (data) {
  // Render partial results
}
```

For more information about error handling, see [Handling operation errors](./error-handling/).
...
> **Note:** `useLazyQuery` does not support SSR environments and will throw an error if the `execute` function is called during server-side rendering. If you need to run a query in an SSR environment, use `useQuery` instead.
...
function DelayedQuery() {
  const [getDog, { loading, error, data, dataState }] = useLazyQuery(GET_DOG_PHOTO);

  if (loading) return <p>Loading...</p>;
  if (error) return <p>Error! {error.message}</p>;
  if (dataState === 'empty') return <p>No dog found</p>;
  if (dataState === 'partial') return <p>Loading more data...</p>;
  if (dataState === 'streaming') return <p>Streaming data...</p>;
...
The `useLazyQuery` hook returns a tuple with two elements:

1. A **query function** that you can call to execute the query
2. A result object that contains the same properties as the `useQuery` hook's result object (`loading`, `error`, `data`, `dataState`, etc.)

The query function accepts only two options:
- `variables`: The variables to pass to the query
- `context`: The context to pass to the query

To change other options (like `fetchPolicy` or `errorPolicy`), you must rerender the component with new options. These options will take effect with the next query execution.

> **Note:** The `defaultOptions` and `initialFetchPolicy` options are no longer supported with `useLazyQuery`. If you need to set default options, pass them directly to the hook instead.
