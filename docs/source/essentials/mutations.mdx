...
Now that we've [learned how to fetch data](/docs/react/essentials/queries/) from our backend with Apollo Client,
...
Both `useMutation` itself and the mutate function accept options that are described in the [API reference](/docs/react/api/react-hooks/#usemutation). Any options you provide to a mutate function _override_ corresponding options
...
object (fully documented in the [API reference](/docs/react/api/react-hooks/#result-2)) include booleans that indicate whether the mutate function has been `called` yet, and whether the mutation's result is currently `loading`.

## Updating the cache after a mutation

When you execute a mutation, you modify back-end data. If that data
is also present in your [Apollo Client cache](/docs/react/advanced/caching/),
...
> Learn more about supported cache functions in [Caching data](/advanced/caching/).
...
> If your mutation specifies an [optimistic response](/features/optimistic-ui/), your update function is called **twice**: once with the optimistic result, and again with the actual result of the mutation when it returns.
...
Learn about all of the fields returned by `useMutation` in the [API reference](/docs/react/api/react-hooks/#result-2).
...
detail with usage examples, see the [API reference](/docs/react/api/react-hooks/#usemutation).
...
- [Local state](/essentials/local-state/): Use Apollo Client to manage the entirety of your application's local state by executing client-side mutations.
- [Caching in Apollo](/advanced/caching/): Dive deep into the Apollo Client cache and how it's normalized. Understanding the cache is helpful when writing update functions for your mutations!
