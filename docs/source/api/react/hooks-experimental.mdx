...
Write queries using `useSuspenseQuery_experimental`. Ensure the component is wrapped with React's [`Suspense`](https://beta.reactjs.org/reference/react/Suspense) component. 
...
function List() {
  const { loading, data } = useSuspenseQuery_experimental(listQuery);
