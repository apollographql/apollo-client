...
A `LocalState` instance containing local resolvers for the `MockedProvider`'s `ApolloClient` instance to use. This replaces the deprecated `resolvers` prop.
...
Apollo Client 4.0 has reorganized its testing utilities to provide better stability and organization. The following changes have been made:

1. Internal testing utilities have been moved from `@apollo/client/testing` to `@apollo/client/testing/internal`
2. Deprecated testing utilities have been removed
3. Testing utilities exported from `@apollo/client/testing` are now considered stable and will not undergo breaking changes

The following APIs have been removed. To migrate, update usages as shown:
...
Additionally, `MockLink` types have been moved to the `MockLink` namespace. This affects the `MockedResponse`, `MockLinkOptions`, and `ResultFunction` types. These types are still exported but are deprecated in favor of the namespace. To migrate, use the types on the `MockLink` namespace instead:
