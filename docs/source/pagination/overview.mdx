...
Instead of recommending a particular pagination strategy, Apollo Client provides flexible cache APIs that help you merge results from a paginated list field, regardless of which pagination strategy your GraphQL server uses. You can even reuse the exact same pagination logic for every list field that uses the same strategy.
