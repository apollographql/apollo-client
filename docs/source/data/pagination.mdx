...
> Remember that the performance motivations for paginating network data do not apply here, because it's so much cheaper to fetch a large array from memory than from the network, and this "entire list" is still probably much shorter than what's available from the server, since it represents only the data your GraphQL client received so far.
