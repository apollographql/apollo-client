...
3.  The final link (called the [_terminating link_](#the-terminating-link)) sends the operation to its destination (usually a GraphQL server over HTTP).
...
You provide the link chain to your Apollo Client instance using the `link` option.
...
Additive composition creates a link chain by executing links in serial order. You use the [`from`](#from) and [`concat`](#concat) helpers to create a link chain using additive composition.

##### `from`

The most common way to compose multiple links together using additive composition is to use the `from` helper. Pass an array of link objects to `from` to execute each link in serial order.
...
Combine two links together using the `concat` method.

```ts
import { concat, ApolloLink, HttpLink } from "@apollo/client";
import MyAuthLink from "../auth";

const additiveLink = concat(
  new MyAuthLink(),
  new HttpLink({ uri: "http://localhost:4000/graphql" })
);

// this also works
const additiveLink = ApolloLink.concat(
  new MyAuthLink(),
  new HttpLink({ uri: "http://localhost:4000/graphql" })
);
```

Each link object includes a `concat` method to combine two links into a link chain. This is useful to combine multiple links together using a single chain of function calls.

The following uses the [`from`](#from) example instead using `concat` to create the link chain.
...
const additiveLink = new RetryLink()
...
You might want your link chain's execution to branch, depending on the details of the operation being performed. You use the `split` function to create a link chain using directional composition by conditionally executing one of two different sub link chain branches.

`split` is comprised of three parts:
...
The following example uses `split` to determine which `HttpLink` to execute depending on the associated context's `version`.
...
The following example adds directional composition to a `SetContextLink` to determine which `HttpLink` should be used as the terminating link.

```ts
new SetContextLink(/* ... */).split(
  (operation) => operation.getContext().version === 1,
  new HttpLink({ uri: "http://localhost:4000/v1/graphql" }),
  new HttpLink({ uri: "http://localhost:4000/v2/graphql" })
);
...
A link object is an instance of the [`ApolloLink` class](https://github.com/apollographql/apollo-client/blob/main/src/link/core/ApolloLink.ts). Each link must define a [_request handler_](#the-request-handler) in order to handle the request.

### The request handler

Every link must define **request handler**. The request handler is a method responsible for performing some logic and executing the request, either by [forwarding](#the-forward-function) the operation to the next link in the link chain, or sending the operation to the destination that executes it, such as a GraphQL server.

This request handler is passed the following arguments:
...
- `forward`: A [function](#the-forward-function) called to execute the next link in the link chain.

Apollo Client executes a GraphQL operation by calling the request handler of the first link in the link chain. Each non-terminating link in the link chain is then responsible for executing its logic and delegating execution down to the next link in the link chain by calling the [`forward` function](#the-forward-function). Once the operation reaches the [terminating link](#the-terminating-link), the request handler sends the operation to the destination and reports the result back up the link chain.
...
You define a request handler for a subclass of `ApolloLink` by overridding the `request` method. Subclassing is typically used to create [stateful links](#stateful-links).
...
When your custom link's request handler is finished performing its logic, it should return a call to the `forward` function and provide the `operation` as an argument. Calling the `forward` function executes the next link in the link chain.

```tsx {5}
const link = new ApolloLink((operation, forward) => {
  // ...Handle the request

  // Execute the next link in the link chain
...
A non-terminating custom link's request handler **must** call `forward(operation)`. If it doesn't, the link is treated as a [terminating link](#the-terminating-link) which could result in the associated GraphQL operation not executed and the request stuck pending indefinitely.

</Caution>

## Handling a response

When your GraphQL server responds with an operation result, that result is passed back up through each link in your link chain:
...
Each link can perform custom logic on the result using [operators](https://rxjs.dev/guide/operators); special functions passed to an Observable's [`pipe`](https://rxjs.dev/guide/operators#piping) function.

The following example uses the RxJS [`map` operator](https://rxjs.dev/api/index/function/map) to modify the result before return it back up the link chain.

```js
import { map } from "rxjs";

const link = new ApolloLink((operation, foward) => {
...
This modification is seen by any _previous_ link in the link chain since the result travels back _up_ the link chain in reverse order.

<Note>

The `map` function is the most common operator for performing modifications to the result. See the RxJS [operators](https://rxjs.dev/guide/operators) documentation for references to other operators that provide other functionality that you might need when building custom links.
...
Stateless links are great for implementing middleware. The following link adds an `Authorization` header to every outgoing request:
...
Stateless links compose nicely for customization using a function:
...
Here's the same `reportErrors` link written as an subclass of `ApolloLink`:
...
The `ReportErrorLink` link is still considered a stateless link despite it implemented as a subclass because it does not maintain state between requests.
...
As an operation moves along your link chain, it maintains a `context` that each link can read and modify. This allows both links and request-based APIs (such as [`useQuery`](../react/useQuery)) to pass metadata along the chain that _other_ links use in their execution logic. Context is not included in the terminating link's request to the GraphQL server or other destination.
...
Mutating the `context` object returned from `operation.getContext()` is not persisted to the context object and will not be seen by downstream links. Use `operation.setContext()` to make modifications to context.
...
It is common to read existing context in order to make modifications to it before writing it back. As a shortcut, you can provide a callback function to `operation.setContext()`. The callback function provides the previous context as an argument and expects the function to return the new context.
...
Context is typically used to communicate between a request API (such as [`useQuery`](../react/useQuery)) and the link chain. You provide context for a particular operation using the `context` option.

The following example provides the initial context with [`useQuery`](../react/useQuery) to define the initial `count` as seen in the previous section's example.
