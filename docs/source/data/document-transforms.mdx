...
Our document transform only needs to modify a field named `currentUser`, so we need to check the field node's `name` node to determine if we are working with the `currentUser` field. Let's add a conditional check and return early if we encounter any field not named `currentUser`.
...
To do so, let's get the field node's `selectionSet` property and loop over its `selections` property to determine if it includes a field named `id`. It's important to note that a `selectionSet` may contain `selections` of both field nodes and fragment nodes. Our implementation only needs to perform checks against a field node, so we also check the selection node's `kind` property. If we find a match on a field named `id`, we can stop traversal of the AST. 

We will bring in both the [`Kind`](https://graphql.org/graphql-js/language/#kind) enum from `graphql-js`, which allows us to compare against the node's `kind` property, and the [`BREAK`](https://graphql.org/graphql-js/language/#break) sentinel, which directs the `visit` function to stop traversal of the AST.
