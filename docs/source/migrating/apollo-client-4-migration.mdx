...
### Typed default options

Certain default options may affect the return types of methods like `client.query` and hooks like `useQuery` or `useSuspenseQuery`. When you set `errorPolicy` or `returnPartialData` in your `ApolloClient` `defaultOptions`, you need to declare them at the type level so that TypeScript can accurately type your results.

#### What changed

When you set certain default options in Apollo Client 3, the TypeScript types for query results did not reflect those defaults. For example:
...
// result.data is typed as TData (not TData | undefined)
const result = await client.query({ query: MY_QUERY });
result.data; // Type: TData
```

In reality, with `errorPolicy: "all"`, `data` can be `undefined` when an error occurs. Apollo Client 4 fixes this by requiring you to declare these default options at the type level.

#### How to migrate

If you use `errorPolicy` or `returnPartialData` in your `defaultOptions`, you need to declare them globally in a TypeScript declaration file:
  ...
  namespace ApolloClient {
    namespace DeclareDefaultOptions {
      interface WatchQuery {
        // Declare your default errorPolicy for watchQuery operations
        errorPolicy: "all";
        // Or declare returnPartialData if you use it
        // returnPartialData: true;
      }
      interface Query {
        // Declare your default errorPolicy for query operations
        errorPolicy: "all";
      }
      interface Mutate {
        // Declare your default errorPolicy for mutation operations
        errorPolicy: "all";
      }
    }
  }
...
Once declared, results will be typed correctly based on your defaults:

```ts
// Now result.data is correctly typed as TData | undefined
const result = await client.query({ query: MY_QUERY });
result.data; // Type: TData | undefined

// If you override the default, the type updates accordingly
const result2 = await client.query({ query: MY_QUERY, errorPolicy: "none" });
result2.data; // Type: TData
```
...
Declaring these options will enforce that you provide values for them in your `defaultOptions`.
If you do not declare these options, you cannot use these properties in the `defaultOptions` you pass to the `ApolloClient` constructor (other `defaultOptions` are still allowed).

#### Opting out

If you create multiple Apollo Client instances with conflicting default options and cannot declare a single global default, you can opt out by declaring the options as `unknown`:
...
This will allow you to call the `ApolloClient` constructor with any arbitrary `defaultOptions`.
In that case, all methods and hooks will assume that these `defaultOptions` are set to the defaults, so you might get incorrect return values.
