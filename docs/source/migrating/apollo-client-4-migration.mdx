...
Using classic signatures after declaring `DeclareDefaultOptions` is not recommended for long-term use. Methods and hooks will produce the same potentially incorrect types as they did before Apollo Client 4.1 and will not reflect the `defaultOptions` you've declared.
