...
Since `useSuspenseQuery` suspends while data is being fetched, a tree of components that all use `useSuspenseQuery` to query different data can cause a "waterfall", where each call to `useSuspenseQuery` depends on the previous to complete before it can be fetched. This can be avoided by fetching with `useBackgroundQuery` and reading the data with `useReadQuery`.
