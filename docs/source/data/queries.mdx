...
If you want to disable automatic re-rendering when the network status changes (for example, to maintain the behavior from Apollo Client v3), you can set `notifyOnNetworkStatusChange` to `false` either in individual queries or globally via `defaultOptions`:
