...
If your paginated field logically represents a _set_ of unique items, you can store it internally using a much more convenient data structure than an array.
...
To demonstrate the flexibility of the field policy system, here's an implementation of the `Query.moreComments` field that uses a map internally:
...
Now there's less ambiguity about where the `cursor` comes from, because it is explicitly stored and returned as part of the query.
...
Instead of a single array, `existing` is now a map from `type` to array, so the same field value can store multiple feed arrays, separated by `type`. However, this manual separation is logically equivalent to what would happen if you moved the `type` argument into `keyArgs` (using `keyArgs: ["type"]`, as above), so the extra effort is probably unnecessary.
