...
Finally, there is a special case to take into consideration. When your client application is running on top of a solution that has server-side rendering, like [NextJS](https://nextjs.org/) for example, you must add an extra validation to check that the runtime in which the apollo client instance is being created is the browser side. and not the server side. Otherwise it won't work.
...
A WebSocket only works on the client side, so if it is on the server side, the HTTP link will be used.
