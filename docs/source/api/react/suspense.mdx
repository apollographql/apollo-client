...
More details on `useSuspenseQuery`'s API can be found in [its API docs](./hooks-experimental#usesuspensequery-api).
...
### Updating state without blocking the UI
...
In these cases, we can use a pair of React APIs designed to allow us to update state without blocking the UI: [`startTransition`](https://react.dev/reference/react/startTransition)/[`useTransition`](https://react.dev/reference/react/useTransition) and [`useDeferredValue`](https://react.dev/reference/react/useDeferredValue). Please see the React documentation for a full set of usage examples and caveats.
...
More details on `useBackgroundQuery` and `useReadQuery`'s API can be found in [their API docs](./hooks-experimental#usebackgroundquery-and-usereadquery).
