...
describe("useQuery Hook", () => {
  describe("General use", () => {
    ...
    it("should keep `no-cache` results when the tree is re-rendered", async () => {
      ...
      const [ProfiledHook, { rerender }] = renderHookToSnapshotStream(
  ...
  describe("polling", () => {
    ...
    it("should start polling when skip goes from true to false", async () => {
      ...
      await expect(stream).not.toRerender({ timeout: 100 });
    ...
    it("should stop polling when component unmounts", async () => {
      ...
      const [stream, { unmount }] = renderHookToSnapshotStream(
  ...
  describe("Error handling", () => {
    ...
    it("should render multiple errors when refetching", async () => {
      ...
      stream.getCurrentSnapshot().refetch().catch(catchFn);
      {
        const result = await stream.takeSnapshot();
        expect(result.loading).toBe(true);
        expect(result.data).toBe(undefined);
    ...
    it("should render data and errors with refetch", async () => {
      ...
      stream.getCurrentSnapshot().refetch();
  ...
  describe("Pagination", () => {
    ...
    it("should only execute single network request when calling fetchMore with no-cache fetch policy", async () => {
      ...
      const { fetchMore } = stream.getCurrentSnapshot();
    ...
    it("uses updateQuery to update the result of the query with no-cache queries", async () => {
      ...
      {
        const { loading, networkStatus, data } = await stream.takeSnapshot();
      ...
      {
        const { data, loading, networkStatus } = await stream.takeSnapshot();
  ...
  describe("Refetching", () => {
    it("refetching with different variables", async () => {
      ...
      stream.getCurrentSnapshot().refetch({ id: 2 });
      {
        const result = await stream.takeSnapshot();
        expect(result.loading).toBe(true);
        expect(result.data).toBe(undefined);
    ...
    describe("refetchWritePolicy", () => {
      ...
      it('should assume default refetchWritePolicy value is "overwrite"', async () => {
        ...
        const thenFn = jest.fn();
        stream.getCurrentSnapshot().refetch({ min: 12, max: 30 }).then(thenFn);
  ...
  describe("Callbacks", () => {
    ...
    it("onCompleted should fire when polling with notifyOnNetworkStatusChange: true", async () => {
      ...
      const cache = new InMemoryCache();
      const onCompleted = jest.fn();
      const [ProfiledHook] = renderHookToSnapshotStream(
        () =>
          useQuery(query, {
            onCompleted,
            notifyOnNetworkStatusChange: true,
  ...
  describe("Optimistic data", () => {
    it("should display rolled back optimistic data when an error occurs", async () => {
      ...
      {
        const { query } = await stream.takeSnapshot();
        expect(query.loading).toBe(true);
      }
      const mutate = stream.getCurrentSnapshot().mutation[0];
      {
        const { query } = await stream.takeSnapshot();
...

      {
        ...
        // rendered.
        let { query, mutation } = await stream.takeSnapshot();
        ...
        while (!mutation[1].loading) {
          ...
          ({ query, mutation } = await stream.takeSnapshot());
        }
      ...
      {
        const { query, mutation } = await stream.takeSnapshot();
  ...
  test("calling `clearStore` while a query is running puts the hook into an error state", async () => {
    ...
    const [stream] = renderHookToSnapshotStream(() => useQuery(query), {
      wrapper: ({ children }) => (
        <ApolloProvider client={client}>{children}</ApolloProvider>
      ),
    });
  ...
  it("does not emit `data` on previous fetch when a 2nd fetch is kicked off and the result returns an error when errorPolicy is none", async () => {
    ...
    {
      const { loading, data, error } = await stream.takeSnapshot();
    ...
    const { refetch } = stream.getCurrentSnapshot();
    ...
    await expect(stream).not.toRerender({ timeout: 200 });
  });
});
