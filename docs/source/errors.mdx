...
Beginning with version 3.8, Apollo Client _omits_ error messages from its core bundle to reduce its memory footprint. Instead, errors direct you to this page to view details.
...
**Note:** The example above checks for the presence of the `__DEV__` variable and loads error messages only if it's present (indicating a dev environment). This can provide a helpful development experience while minimizing your memory footprint in production.
