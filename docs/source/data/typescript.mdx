...
Certain default options may affect the return types of methods like `client.query` and hooks like `useQuery` or `useSuspenseQuery`.
When you set `errorPolicy` or `returnPartialData` in your `ApolloClient` `defaultOptions`, you need to declare them at the type level so that TypeScript can accurately type your results.

### Why declare default options?
...
```ts
// With errorPolicy: "all" declared as default
const result = await client.query({ query: MY_QUERY });
result.data; // Type: TData | undefined
result.error; // Type: ErrorLike | undefined

// Override the default to get different types
const result2 = await client.query({ query: MY_QUERY, errorPolicy: "none" });
result2.data; // Type: TData
result2.error; // Type: never
```
...
If you do not declare these options, TypeScript will prevent you from using these specific fields (for example, `errorPolicy` or `returnPartialData`) in the `ApolloClient` constructor's `defaultOptions`, but you can still provide other default options.

If you cannot declare specific global defaults (for example, if you have multiple Apollo Client instances with different default options), you can opt out by declaring the options as `unknown`:
