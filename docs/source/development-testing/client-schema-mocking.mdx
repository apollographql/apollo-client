...
There you have it, a workflow for developing new features with new schema fields ahead of the actual schema implementation. For further reading, visit the [full-stack quickstart course](https://www.apollographql.com/tutorials/fullstack-quickstart/managing-local-state) which describes how to use client fields to manage local state in greater depth.
