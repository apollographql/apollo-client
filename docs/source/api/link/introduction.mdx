...
The `forward` function's return type is an `Observable` provided by the [`rxjs`](https://github.com/ReactiveX/rxjs) library. See the `rxjs` documentation for details.
