...
By default, `CombinedProtocolErrors` formats the `message` property by joining each error's `message` field with a newline. If you'd like to customize the format of the `message`, such as changing the delimiter or adding a message prefix, override the static `formatMessage` method.
...
<Note>
  ...
  by Apollo Client; otherwise, the default message formatter is. We
