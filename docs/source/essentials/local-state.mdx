...
1. Use the same query/mutations and hooks/components as we do with remote data
		- Differentiate local from remote using the @client directive
		- Still use standard GraphQL syntax
2. Local 'store/state' is held in the same cache as remote data, by default
...
3. Handle logic for updating local cache using resolvers or the client directly
	    - Using cache.writeData, cache.writeQuery and/or cache.writeFragment
	    - cache.writeQuery and cache.writeFragment ensure our writes use the same
		  structure as the corresponding query/fragment.
...
const [ addItemToCart,  { data } ] = useMutation(MUTATION_ADD_ITEM_TO_CART)
// or using the same components as before..
<Mutation mutation={MUTATION_ADD_ITEM_TO_CART}>
```
...
In this section, we'll explain how local queries work in detail. We'll explore the inner-workings of a local query and then provide some solutions to more specific use cases.
  1. [Initializing our cache:](#initializing-our-cache)
	  - Why it's needed and how do we do it?
  2. [Local data query flow](#local-data-query-flow)
	  - [Handling `@client` fields with the cache](#handling-client-fields-with-the-cache)
	  - [Handling `@client` fields with resolvers]
  3. [Additional Concepts](#additional-concepts)
	  - [Handling fetch policies](#handling-fetch-policies)
	  - [Force resolvers on every request](#force-resolvers-on-every-request)
      - [`@client` results as variables](#using-client-results-as-variables)

### Initializing our cache
---
...
Let's assume we're starting with an empty cache and talking to a network based GraphQL API. Using the default fetchPolicy in our query ("cache-first" for all queries, local & remote), Apollo Client will go through the following steps:
  - First, search the cache for a matching result to our query, ignoring all `@client` directives, but still checking for the respective field of the query. It is important to note that this is the same cache that stores data retrieved from remote requests.
  - If data is found in the cache, it is returned immediately and no local (or remote) resolvers will be run.
  - If one is not found, then apollo client basically splits the query into two parts: fields with `@client` and fields without.
    - The fields with the `@client` directive are sent to the local resolvers to "resolve" their value.
    - The fields without `@client` are sent to the server to be "resolved."
  - Once the entire query is resolved, the local and remote data is merged and then written back to the cache and returned as the response to the original operation.

We can adjust the way our values are resolved (via fetchPolicies, etc.) and I encourage you to read the [pros and cons below](#advanced-concepts).
...
### Additional concepts
---
There are some additional use cases that could potentially help your specific application. Not everyone will need these, but are definitely important to consider. They are currently as follows:
  a. [Handling fetch policies](#handling-fetch-policies)
  b. [Force resolvers on every request using @client(always: true)](#force-resolvers-on-every-request)
  c. [Using @client results as variables](#using-client-results-as-variables)

#### Handling fetch policies
...
#### Using @client results as variables
...
## Updating the Store
To update our store, which recall is the in the cache, we have a couple main ways. Also, recall that the cache that keeps track of our local data, is the exact same cache that handles our remote data. Here are the main ways we can update the store.
1. [Direct writes](#direct-writes), using useApolloClient hook, or ApolloConsumer component
	- as when we ["initialized the cache"](#initializing-our-cache)
2. [Mutations which execute local resolvers (most often preferred)](#mutations-that-execute-local-resolvers)
	  - Same mutations as normal, with the exception of the client directive as [shown earlier](#mutations)
	  - Mutations call resolvers, which update the cache. The components subscribed to that part of the cache will then be updated as well.
      - Resolvers are defined in a "resolver map." This is simply an object with resolver functions for each GraphQL type, which we pass to our ApolloClient constructor.
      - Resolvers handle the logic to actually update our cache with the correct data.
      - Resolvers access the cache via the context parameter in our resolvers. Please review the resolver signature/parameters in the API Overview [section here](#resolver-parameters).

You can think of resolvers in a similar manner to reducers in redux. Rather than reading the state and returning a new state, we are usually reading the cache and writing back to the cache. It's also useful to think of a GraphQL query or mutation as a tree of function calls for each field. These function calls resolve to data or another function call. So when a GraphQL query is run through Apollo Client, it looks for a way to essentially run functions for each field in the query. When it finds an `@client` directive on a field, it turns to its internal resolver map looking for a function it can run for that field.
...
### Mutations that execute local resolvers
...
First, we create a GraphQL mutation that takes the item's id we want to add to our cart as its only argument. We indicate that this is a local mutation by marking the field with a `@client` directive. This will tell Apollo Client to call our local `addItemToCart` mutation resolver in order to resolve the field. Then, we create a component with `useMutation` hook or `Mutation` component just as we would for a remote mutation. Finally, pass in your GraphQL mutation to your component and trigger it from within the UI.
...
#### Resolvers
Let's take a look at the resolver map and a the addItemToCart resolver that will actually add the item to our cache. The comments provide a good step-by-step for the logic, but the signature/parameters will be new if you haven't created GraphQL resolvers before. To help make local resolvers more flexible, the signature of a resolver function is the exact same as resolver functions on the server built with [Apollo Server](https://www.apollographql.com/docs/apollo-server/essentials/data). You can recap the [parameters here](#resolver-parameters).
...
---
### Cache Methods
There are three different ways we can write to our cache and two ways to read from it, which we will cover now. We access the cache from the third parameter in our resolver functions.

1. [writeData](#cache.writedata)
2. [writeQuery/readQuery](#writequery-and-readquery)
3. [writeFragment/readFragment](#writefragment-and-readfragment)
...
3. `defaults` are no longer supported. To prep the cache, use [`cache.writeData`](#cache.writedata) directly instead. So
...
3. `context`: An object of contextual information shared between your React components and your Apollo Client network stack. In addition to any custom context properties that may be present, local resolvers always receive the following:
    ...
    - `context.cache`: The Apollo Cache instance, which can be used to manipulate the cache with `.readQuery`, `.writeQuery`, `.readFragment`, `.writeFragment`, and `.writeData`
