...
Certain default options might affect the return types of methods like `client.query` and hooks like `useQuery` or `useSuspenseQuery`. When you set `errorPolicy` or `returnPartialData` in your `ApolloClient` `defaultOptions`, define them at the type level so TypeScript can accurately type your results.
...
Defining these options ensures you provide values for them in your `defaultOptions`.
...
If you create multiple Apollo Client instances with conflicting default options and can't define a single global default, you can opt out by defining the options as `unknown`:
