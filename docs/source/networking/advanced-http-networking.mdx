...
Apollo Client 4.0 strictly adheres to the [GraphQL over HTTP spec](https://graphql.github.io/graphql-over-http/draft/). This includes:

1. Support for the `application/graphql-response+json` media type
2. Modified behavior for the `application/json` media type
3. Updated default `Accept` header to `application/graphql-response+json,application/json;q=0.9`

The client will:
- Parse the response as a well-formed GraphQL response when the server encodes `content-type` using `application/graphql-response+json` with a non-200 status code
- Throw a `ServerError` when the server encodes `content-type` using `application/json` and returns a non-200 status code
- Throw a `ServerError` when the server encodes using any other `content-type` and returns a non-200 status code
...
The `ServerError` type has been updated in Apollo Client 4.0. The `result` property has been replaced with `bodyText`, which contains the raw string body. `HttpLink` and `BatchHttpLink` no longer try to parse the response body as JSON when a `ServerError` is thrown.
...
const errorLink = onError(({ networkError }) => {
  if (networkError) {
    // networkError.bodyText contains the raw response body
    console.error('Network error:', networkError.bodyText);
  }
});
