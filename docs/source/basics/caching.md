...
```
Be carefull with creating identifiers without prefixes like `__typename` as they are prone to name clashes between various types.

You can use different unique identifiers for different data types by keying off of the `__typename` property attached to every object typed by GraphQL. Or fallback to default handler `defaultDataIdFromObject`. For example:
...
const cache = new InMemoryCache({
  dataIdFromObject: object => {
    switch (object.__typename) {
      case 'foo': return `foo:${object.key}`; // use `foo` prefix and `key` as the primary key
