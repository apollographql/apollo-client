...
Is similar to `performTransaction` but takes named options rather than positional parameters.
 The batch method is intended to replace both `performTransaction` and `recordOptimisticTransaction` Subclasses of ApolloCache (such as `InMemoryCache`) can override the batch method to do more interesting things with its options.
...
To find out which watched queries were invalidated during
