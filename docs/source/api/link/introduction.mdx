...
Changes to the `context` object returned from `operation.getContext()` is not persisted, and downstream links won't see them. Use `operation.setContext()` to make modifications to context.
