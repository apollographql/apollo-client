...
However, _we_ are confident that these two objects represent the same author, because a book's author virtually never changes. This common-sense knowledge is something we have at our disposal, as humans, but it must be communicated to the cache, which is neither human nor capable of telepathy.

In situations like this one, we can define a custom `merge` function for the `author` field within the type policy for `Book`:
