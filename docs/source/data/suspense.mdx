...
Note that with both error policies of `ignore` and `all`, when encountering a network error Apollo Client's suspense hooks will always throw an error wherever the data is being read (i.e. the component calling `useSuspenseQuery` or `useReadQuery`).
...
__In rare cases__, you may want to avoid suspending in one component when other components are (re-)fetching the same data with the same variables. To do this, you can use the `queryKey` option provided to either `useSuspenseQuery` or `useBackgroundQuery` to uniquely identify a query. When `queryKey` is provided, Apollo Client will use it to identify a query in combination with the `DocumentNode` and `Variables`. For more information, see [`useSuspenseQuery`](../api/react/hooks/#usesuspensequery) or [`useBackgroundQuery`](../api/react/hooks/#usebackgroundquery-and-usereadquery)'s API docs.

### `refetch` and `fetchMore`

When using `useBackgroundQuery` to initiate a fetch in the background, both `refetch` and `fetchMore` functions can be destructured from the second item in the tuple returned by the hook.
...
`useSuspenseQuery` also returns `refetch` and `fetchMore` functions in addition to `subscribeToMore`. Let's take a look at an example:
