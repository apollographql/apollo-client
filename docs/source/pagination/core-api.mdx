...
By default, `fetchMore` executes a query with the exact same shape as your original query. You can pass new values for the query's `variables` (such as providing a new `offset`) like so:
