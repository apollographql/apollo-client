...
```
A point to note here is the query will be executed again if any of the `options` passed to the `useLazyQuery` hook changes. This is the common behaviour of `useLazyQuery ` and `useQuery`.
