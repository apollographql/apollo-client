...
import CachedMutationOptions from '../../../shared/cached-mutation-options.mdx';
...
function AddTodo() {
  let input;
  const [addTodo, { data }] = useCachedMutation(ADD_TODO, FRAGMENT, {
    mutationName: 'AddTodo',
    rootCacheId: 'todos'
  });

  /*
  The updateKey option should be informed like this:
