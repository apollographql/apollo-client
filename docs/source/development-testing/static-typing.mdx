...
In Apollo Client, all APIs that accept a `DocumentNode` can alternatively take a `TypedDocumentNode<Data, Variables>`. This type enables Apollo Client APIs to infer the data and variable types using the GraphQL document.
