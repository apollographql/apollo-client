...
To help handle this case, we can [set key arguments](#setting-key-args) for the field.
...
After setting `keyArgs` as shown, cache entries for `Query.feed` look like this (note the absence of `offset` and `limit`):
