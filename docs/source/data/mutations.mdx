...
- A **mutate function** that you can call at any time to execute the mutation
  - Unlike `useQuery`, `useMutation` _doesn't_ execute its operation automatically on render. Instead call the `mutate` function run execute the mutation.
...
<PropertySignatureTable
  canonicalReference="@apollo/client!useMutation.Options:interface"
