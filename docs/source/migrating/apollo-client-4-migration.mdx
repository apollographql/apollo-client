...
#### Changes to promise resolution from the `execute` function

The promise returned from the `execute` function is now consistently resolved or rejected depending on the configured `errorPolicy`. Errors returned while using an `errorPolicy` of `none` always rejects the promise, regardless of the error type. Errors returned while using an `errorPolicy` of `all` always resolves the promise and sets the `error` property on the result, regardless of the error type. Errors returned while using an `errorPolicy` of `ignore` discards them and resolves the promise, regardless of the error type.
