...
You can also explicitly opt into modern signatures without declaring any `defaultOptions`. This is useful if you want accurate type inference and don't use `defaultOptions`:
...
If you've declared `DeclareDefaultOptions` (which activates modern signatures automatically) but need to temporarily keep classic signatures for some code during migration, you can explicitly force classic signatures:
...
Using classic signatures after declaring `DeclareDefaultOptions` is not recommended for long-term use. In this configuration, Apollo Client's methods and hooks produce the same potentially incorrect types as they did before Apollo Client 4.1—they do not reflect the `defaultOptions` you've declared.
