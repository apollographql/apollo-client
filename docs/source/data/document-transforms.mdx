...
Have you noticed that Apollo Client makes modifications to your queries, such as adding the `__typename` field, before those queries are sent to your GraphQL server? Apollo Client includes an advanced capability that allows you to define your own GraphQL document transforms that make modifications to your GraphQL queries. Let's learn how you can make and utilize your own GraphQL document transforms.

> This feature requires Apollo Client `v3.8` or greater.

## Prerequisites

This article assumes you're familiar with the concept of an [AST](https://en.wikipedia.org/wiki/Abstract_syntax_tree). Document transforms make modifications to the GraphQL AST, which is parsed from a raw GraphQL query string using the `gql` function. A GraphQL document is a GraphQL string that defines one or more operations and fragments.

## Writing your own document transform

You create your own document transforms using the `DocumentTransform` class. The created transform is then passed to the `ApolloClient` constructor.
