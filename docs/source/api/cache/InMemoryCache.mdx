...
Updates data in the cache in the shape of a provided GraphQL query. Returns the resulting updated result or `null` if the write failed.

If `undefined` is returned from the update function, nothing will be written to the cache.
...
Takes an `options` object and a callback function as parameters. Supported fields of this object are described below.
...
An update function takes a cached query’s current value and returns the value that should replace it, or `undefined` if no change should be made.

The first parameter passed to the update function is the current cached value of the query.
...
Updates data in the cache in the shape of a provided GraphQL fragment:
...
An update function takes a cached fragment’s current value and returns the value that should replace it, or `undefined` if no change should be made.

The first parameter passed to the update function is the current cached value of the fragment.
