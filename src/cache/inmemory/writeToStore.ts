...
export class StoreWriter {
  ...
  private processSelectionSet({
    ...
    const [id, keyObject] = policies.identify(
      result, selectionSet, context.fragmentMap, disableNormalization);
    ...
    selections.forEach(selection => {
      ...
      if (isField(selection)) {
        ...
        if (value !== void 0) {
          ...
          let incomingValue =
            this.processFieldValue(value, selection, context, childTree, disableNormalization);
    ...
    if (Array.isArray(value)) {
      return value.map((item, i) => {
        const value = this.processFieldValue(
          item, field, context, getChildMergeTree(mergeTree, i), disableNormalization);
