...
In order to change the data in your store call methods on your `DataProxy` instance like [`writeQuery`](/local-data/caching/#writequery-and-writefragment) and [`writeFragment`](/local-data/caching/#writequery-and-writefragment). This will update your cache and reactively re-render any of your GraphQL components which are querying affected data.

To read the data from the store that you are changing, make sure to use methods on your `DataProxy` like [`readQuery`](/local-data/caching/#readquery) and [`readFragment`](/local-data/caching/#readfragment).

For more information on updating your cache after a mutation with the `options.update` function make sure to read the [Apollo Client technical documentation on the subject](/local-data/caching/#updating-after-a-mutation).
