...
> Refer to the [Mutation](../../data/mutations/) section for a more in-depth overview of `useMutation`.
...
```jsx
import { gql, useCachedMutation } from '@apollo/client';
...
function AddTodo() {
  let input;
  const [addTodo, { data }] = useCachedMutation(ADD_TODO, FRAGMENT, 'AddTodo', 'todos');
...
| Param      | Type         | Description                                                      |
| ---------- | ------------ | ---------------------------------------------------------------- |
| `fragment` | DocumentNode | A GraphQL fragment document parsed into an AST by `gql`. |

#### `mutationName`

| Param      | Type         | Description                                                      |
| ---------- | ------------ | ---------------------------------------------------------------- |
| `mutationName` | string | The name of the mutation. |

#### `cacheId`

| Param      | Type         | Description                                                      |
| ---------- | ------------ | ---------------------------------------------------------------- |
| `cacheId` | string | The id of the item into the cache. You can more about it [here](../cache/InMemoryCache#input) |

#### `options`

The options for this hooks is the same options that you can pass to [useMutation](./hooks#options-2) hook. The only difference is
that you don't need to pass the `update` property.

### Result

The result of this hooks is the same of [useMutation](./hooks#result-2) hook result
...
```ts
function useCachedMutation<TData = any, TVariables = OperationVariables>(
  mutation: DocumentNode,
  options?: MutationHookOptions<TData, TVariables>,
): MutationTuple<TData, TVariables> {}
```
