...
> ⚠️ **DEPRECATION WARNING:** Local resolvers are still available in Apollo Client 3, but they are deprecated. We recommend using [field policies](./managing-state-with-field-policies) instead. Local resolver support will be removed in a future major Apollo Client release.
>
> See the [deprecation notice](#deprecation-notice) for details.
