...
By default, the `useQuery` hook checks the Apollo Client cache to see if each field you requested is already available locally. If a field _is_ available locally, `useQuery` _omits_ that field in the query it sends to your GraphQL server. This `cache-first` policy is Apollo Client's default **fetch policy**.

You can optionally specify a different fetch policy for a query. To do so, include the `fetchPolicy` option in your call to `useQuery`:
...
Apollo Client does _not_ query your GraphQL server for any requested fields that are already available in the cache. If _all_ requested fields are available in the cache, Apollo Client doesn't query your server at all.
...
Apollo Client executes the query against both the cache _and_ your GraphQL server. The query automatically updates if the result of the server-side query modifies cached fields.
</td>
</tr>
