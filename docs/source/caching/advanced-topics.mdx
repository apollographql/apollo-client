...
To use the `@connection` directive, add it to field you want a custom cache key for. The directive requires a `key` parameter to specify the custom cache key. You can optionally include the `filter` parameter, which takes an array of query argument names to include in the generated custom cache key.
...
> Note: Because this example uses the `type` argument in the cache key, it doesn't need to provide `offset` or `limit` arguments.
