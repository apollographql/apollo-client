...
The `graphql()` function will only be able to provide access to your GraphQL data if there is a [`<ApolloProvider/>`](./react-common/#apolloprovider) component higher up in your tree to provide an [`ApolloClient`](./apollo-client/) instance that will be used to fetch your data.
...
The `data.fetchMore` function allows you to do pagination with your query component. To learn more about pagination with `data.fetchMore`, be sure to read the [pagination](../data/pagination/) recipe which contains helpful illustrations on how you can do pagination with React Apollo.
...
For more information on updating your cache after a mutation with the `options.update` function make sure to read the [Apollo Client technical documentation on the subject](../caching/cache-interaction/#updating-after-a-mutation).
...
A simple enhancer which provides direct access to your [`ApolloClient`](./apollo-client/) instance. This is useful if you want to do custom logic with Apollo. Such as calling one-off queries. By calling this function with the component you want to enhance, `withApollo()` will create a new component which passes in an instance of `ApolloClient` as a `client` prop.
...
This will only be able to provide access to your client if there is an [`<ApolloProvider/>`](./react-common/#apolloprovider) component higher up in your tree to actually provide the client.
