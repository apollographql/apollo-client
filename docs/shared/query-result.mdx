...
The followup execution's `fetchPolicy` is set to `network-only` if the original query's `fetchPolicy` isn't guaranteed to send a network request (such as with `cache-first`).
