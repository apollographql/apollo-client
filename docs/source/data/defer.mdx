...
When our call to the `useQuery` hook first resolves with an initial payload of data, `firstName` and `lastName` will be populated with the values from the server. We can render these fields as we normally would. **Our deferred fields will not exist as keys on `data` yet**, so we must add conditional logic that checks for their presence.
