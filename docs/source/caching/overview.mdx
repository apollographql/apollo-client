...
Apollo Client's `InMemoryCache` is a **flat lookup table** of objects that can reference each other. These objects are the same objects that are returned by your GraphQL queries.
...
Normalization constructs a partial copy of your data graph on your client, in a format that's optimized for reading and updating as your app's state changes.
